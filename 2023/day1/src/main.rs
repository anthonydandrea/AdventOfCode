fn main() {
    ...
    let numeric_value = |line: &str, rev: bool| -> String {
        ...
        while 0 <= idx && idx < line.len() as i32 {
           if line.get(idx as usize..idx as usize +1).unwrap().matches(char::is_numeric).peekable().peek().is_some() {
               return line.get(idx as usize..idx as usize + 1).unwrap().to_owned();
           }

           for (k, v) in map.iter() {
                if idx + k.len() as i32 <= line.len() as i32 && line.get(idx as usize..idx as usize + k.len()).unwrap() == *k {
                    return (*v).to_owned();
                }
           }
    ...
    let val: i32 = input_split.filter(|l| !l.is_empty()).map(|line| {
        // println!("{line:?}");
